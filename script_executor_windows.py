...
def cargar_total_time(segundos):
    ...
    for file in glob.glob("*"):
        if file == '{}sTotalTime.png'.format(segundos):
            load_graphic('{}sTotalTime.png'.format(segundos))
            return

    # Si no lo están, las generamos

    ## Ejecutamos el script que genera las gráficas
    subprocess.call("python time-test.py {}".format(segundos), shell=True)
...
def cargar_mail_ip():
    ...
    ## Ejecutamos el script que genera las gráficas
    subprocess.call("python time-test.py", shell=True)
...
btn = Button(window, text="Obtener logs", bg="white", fg="black",command=run)
btn.place(width = 100, height = 50, x = 350, y = 680)
